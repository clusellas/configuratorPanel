...
const ImageGridFaldon = ({ elements, onImageClick }) => {
    ...
    const filteredElements = elements.filter(
        element => element.faldon.image !== 'http://localhost:8000/media/default.png'
    );
                ...
                <Grid container item spacing={2} key={rowIndex}>
                    {row.map((element, elementIndex) => (
                        <Grid item xs={12} sm={6} md={4} key={elementIndex}>
                            <ImageContainer onClick={() => onImageClick(element.faldon)}>
                                <img
                                    src={element.faldon.image}
                                    alt={element.faldon.code}
                                    style={{ maxWidth: '100%', maxHeight: '700px', objectFit: 'cover' }}
                                />
                                <Typography variant="body1">{element.faldon.name}</Typography>
                            </ImageContainer>
                        </Grid>
                    ))}
                </Grid>
