...
function CompositionView() {
    ...
    const fetchData = async () => {
        try {
            ...
            if (
                composition.encimera &&
                composition.encimera.articulo.attr.plana === true
            ) {
                setshowLavabo(true);
            }
        } catch (error) {
    ...
    function findIndexSelectedOptionInLinea() {
        if (!selectedOption || !currentCO) {
            ...
            for (let i = 0; i < currentCO.current_linea.opciones.length; i++) {
                if (
                    currentCO.current_linea.opciones[i].id === selectedOptionId
                ) {
                    return i;
                }
            }
    ...
    function navigation(direction) {
        if (!selectedOption || !currentCO) {
            ...
            if (
                index === -1 ||
                index === currentCO.current_linea.opciones.length
            ) {
                return;
            }
